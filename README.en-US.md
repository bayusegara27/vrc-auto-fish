...
- Pushing a tag such as `v1.2.3` runs `.github/workflows/release.yml`, builds the Windows package, and uploads a release `7z` archive automatically.
- The release archive contains the packaged `VRC auto fish-CUDA.exe` plus bundled runtime resources such as images, translations, model files, and required native dependencies.
...
- `.github/workflows/release.yml`: builds a Windows release on `v*` tags (or manual dispatch) and uploads a `7z` archive to GitHub Releases
